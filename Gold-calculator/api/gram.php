...
<head>
    ...
    <title>Gold Calculator-Gram</title>
    <style>
body {
    margin: 0;
    padding: 0;
...
<?php
    ...
    if (isset($_POST["grams"]) && isset($_POST["sale_price"]) && isset($_POST["current_price"])) {
        $grams = $_POST["grams"];
        $sale_price = $_POST["sale_price"];
        $current_price = $_POST["current_price"];

        $gram_to_pae = $grams / 1.0205;
        $pae_to_kyat_tharr = $gram_to_pae / 16;
        $gold_price = $pae_to_kyat_tharr * $current_price;
        $hand_price = $sale_price - $gold_price;

        $gram_to_pae = number_format($gram_to_pae, 2);
...
<div class="cal-container">
    <h1>Gold Calculator(Gram)</h1>
    <form action="gram.php" method="post">
        <div>
            <label for="weightIngram">
                Weight in gram
            </label>
            <input type="number" name="grams" id="weightIngram" value="<?php echo $grams; ?>" required>
        </div>
        <div>
            <label for="saleprice">
                Sale Price
    ...
    <div class="result-container">
        <h2>Results</h2>
        <p>‌‌ရွှေအလေးချိန် = <?php echo $gram_to_pae; ?> ပဲ</p>
